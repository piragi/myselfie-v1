...
uint64_t is_shift();
uint64_t is_and_operator();
uint64_t is_or_operator();
uint64_t is_not_operator();
...
void emit_xori(uint64_t rd, uint64_t rs1, uint64_t rs2);
...
void init_disassembler() {
  ...
  MNEMONICS = smalloc((XORI + 1) * SIZEOFUINT64STAR);
...
void create_array_dimension(uint64_t* head, uint64_t dimension){
  ...
  new_entry = allocate_array_dimensions_entry();

  while(get_next_dimension(head) != (uint64_t*) 0)
    head = get_next_dimension(head);


  set_dimension(new_entry, dimension);
...
void mul_array_dimension(uint64_t* head){
  ...
  while(get_next_dimension(head) != (uint64_t*) 0){
    head = get_next_dimension(head);
    talloc();
    emit_addi(current_temporary(), REG_ZR, get_dimension(head));
...
uint64_t compile_and_expression() {
  ...
  ltype = compile_compare_expression();

  while (is_and_operator()) {
    operator_symbol = symbol;

    get_symbol();

    rtype = compile_compare_expression();
  ...
  // type of and expression is grammar attribute
  return ltype; 
}
uint64_t compile_expression() {
  uint64_t ltype;
  ...
  ltype = compile_and_expression();

  while (is_or_operator()) {
    operator_symbol = symbol;

    get_symbol();

    rtype = compile_and_expression();
...
uint64_t compile_compare_expression() {
  ...
  // type of compare expression is grammar attribute
  return ltype;
}
          ...
          while (symbol == SYM_LBRACKET){
            ...
            if (symbol == SYM_INTEGER){
              ...
              if (symbol == SYM_RBRACKET){
                ...
                range_array = range_array * literal;
              }
            }
          }

          offset = offset + range_array;
          type = UINT64ARRAY_T;
        }
      }
    } else if (symbol == SYM_ASTERISK) {
...
void print_lui() {
  ...
  printf3("%s %s,%x", get_mnemonic(is), get_register_name(rd), (char*) sign_shrink(imm, 20));
}
...
void print_addi() {
  ...
  printf4("%s %s,%s,%d", get_mnemonic(is), get_register_name(rd), get_register_name(rs1), (char*) imm);
}
...
void do_xori() {
  ...
  if (rd != REG_ZR) {
    ...
    while(i <= 32){
      ...
      if(get_bits(*(registers + rs1), 0, i) == get_bits(*(registers + rs1), 0, i-1)){
        if(get_bits(imm, 0, i) != get_bits(imm, 0, i-1)){
          next_rd_value = next_rd_value + two_to_the_power_of(i-1);
        }
      }
      else if (get_bits(*(registers + rs1), 0, i) != get_bits(*(registers + rs1), 0, i-1))
        if(get_bits(imm, 0, i) == get_bits(imm, 0, i-1)){
          next_rd_value = next_rd_value + two_to_the_power_of(i-1);
        }
...
void do_and() {
  ...
  if (rd != REG_ZR) {
    ...
    while(i <= 32){
      ...
      if (get_bits(*(registers + rs1), 0, i) != get_bits(*(registers + rs1), 0, i-1))
        if(get_bits(*(registers + rs2), 0, i) != get_bits(*(registers + rs2), 0, i-1)){
          next_rd_value = next_rd_value + two_to_the_power_of(i-1);
        }
...
void do_or() {
  ...
  if (rd != REG_ZR) {
    ...
    while(i <= 32){
      ...
      else if (get_bits(*(registers + rs2), 0, i) != get_bits(*(registers + rs2), 0, i-1))
        next_rd_value = next_rd_value + two_to_the_power_of(i-1);
...
void print_load() {
  print_code_context_for_instruction(pc);
  printf4("%s %s,%d(%s)", get_mnemonic(is), get_register_name(rd), (char*) imm, get_register_name(rs1));
}
...
void print_store() {
  print_code_context_for_instruction(pc);
  printf4("%s %s,%d(%s)", get_mnemonic(is), get_register_name(rs2), (char*) imm, get_register_name(rs1));
}
...
void print_beq() {
  print_code_context_for_instruction(pc);
  printf4("%s %s,%s,%d", get_mnemonic(is), get_register_name(rs1), get_register_name(rs2), (char*) signed_division(imm, INSTRUCTIONSIZE));
...
void print_jal() {
  print_code_context_for_instruction(pc);
  printf3("%s %s,%d", get_mnemonic(is), get_register_name(rd), (char*) signed_division(imm, INSTRUCTIONSIZE));
  if (disassemble_verbose)
    printf1("[%x]", (char*) (pc + imm));
}
...
void print_jalr() {
  print_code_context_for_instruction(pc);
  printf4("%s %s,%d(%s)", get_mnemonic(is), get_register_name(rd), (char*) signed_division(imm, INSTRUCTIONSIZE), get_register_name(rs1));
}
...
void print_data(uint64_t data) {
  ...
  if (IS64BITSYSTEM)
    printf1(".8byte %x", (char*) data);
  else
    printf1(".4byte %x", (char*) data);
}
...
void decode() {
  ...
  if (opcode == OP_IMM) { //could be ADDI, XORI
    decode_i_format();
